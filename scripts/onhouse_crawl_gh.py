...
# -*- coding: utf-8 -*-
"""
온하우스 전체 매물 크롤링 + wishes.co.kr 업로드
GitHub Actions에서 실행되는 스크립트
"""
import json, time, re, sys, os, urllib.request, urllib.parse, urllib.error, http.cookiejar
    ...
    for i in range(N):
        for j in range(N):
            p = {
                ...
                'roomTypeList':'["6","7","8","9","10","11"]','page':'1','listNum':'20',
            }
        ...
        if done % 100 == 0:
            print(f"Grid {done}/{total}, IDs: {len(ids)}", flush=True)
    print(f"Total IDs: {len(ids)}")
    return list(ids)

def g(pat, html, flags=re.DOTALL):
...
def parse(op, lid):
    ...
    title = g(r'<h1[^>]*class="[^"]*title[^"]*"[^>]*>(.*?)</h1>', html)
    if title: title = re.sub(r'<[^>]+>','',title).strip()
    d['title'] = title or g(r'"title"\s*:\s*"([^"]+)"', html) or f"온하우스 {lid}"
    d['address'] = (g(r'"address"\s*:\s*"([^"]+)"', html) or g(r'"jibunAddr"\s*:\s*"([^"]+)"', html) or '')
    raw_type = g(r'"roomType"\s*:\s*"([^"]+)"', html) or '상가'
    d['type'] = TYPE_MAP.get(raw_type.strip(), '상가')
    dep = int(g(r'"deposit"\s*:\s*(\d+)', html) or 0) * 10000
    mon = int(g(r'"monthly"\s*:\s*(\d+)', html) or 0) * 10000
    sale = int(g(r'"salePrice"\s*:\s*(\d+)', html) or 0) * 10000
    rent_raw = g(r'"dealType"\s*:\s*"([^"]+)"', html) or ''
    tx = '전세' if '전세' in rent_raw else ('매매' if '매매' in rent_raw else '월세')
    d['transaction_type'] = tx
    d['deposit'] = dep; d['monthly_rent'] = mon if tx!='매매' else 0
    d['sale_price'] = sale if tx=='매매' else None
    area = g(r'"exclusiveArea"\s*:\s*"?([\d.]+)"?', html)
    d['area_m2'] = float(area) if area else 0.0
    floor = g(r'"floor"\s*:\s*"?(\d+)"?', html)
    d['floor'] = int(floor) if floor else None
    mf = g(r'"manageFee"\s*:\s*"?(\d+)"?', html)
    d['maintenance_fee'] = int(mf)*10000 if mf else 0
    d['maintenance_includes'] = []
    d['building_name'] = g(r'"buildingName"\s*:\s*"([^"]+)"', html)
    d['previous_business'] = None
    d['recommended_business'] = None
    d['restricted_business'] = None
    park = g(r'"parkingNum"\s*:\s*"?(\d+)"?', html)
    d['parking_spaces'] = int(park) if park else None
    d['contact'] = g(r'"phone"\s*:\s*"([^"]+)"', html) or g(r'"tel"\s*:\s*"([^"]+)"', html)
    lat = g(r'"lat(?:itude)?"\s*:\s*"?(3[67]\.\d+)"?', html)
    lng = g(r'"ln?g(?:itude)?"\s*:\s*"?(12[67]\.\d+)"?', html)
    d['latitude'] = float(lat) if lat else None
    d['longitude'] = float(lng) if lng else None
    imgs = list(dict.fromkeys(re.findall(r'https?://[^\s"'<>]+onhouse[^\s"'<>]*\.(?:jpg|jpeg|png|webp)', html, re.I)))[:10]
    d['images'] = imgs
    year = g(r'"builtYear"\s*:\s*"?(\d{4})"?', html)
...
def main():
    ...
    for i, lid in enumerate(new_ids):
        detail = parse(op, lid)
        if not detail:
            print(f"[{i+1}/{len(new_ids)}] {lid} parse FAIL", flush=True)
            failed += 1; time.sleep(0.5); continue
        result = upload(detail)
        if 'error' in result:
            print(f"[{i+1}/{len(new_ids)}] {lid} upload FAIL: {result['error']}", flush=True)
            failed += 1
        else:
            print(f"[{i+1}/{len(new_ids)}] {lid} OK (db={result.get('id','?')})", flush=True)
            uploaded += 1
        if (i+1) % 50 == 0:
            op = login()
        time.sleep(0.4)
