...
### I-CDN-2: vercel.json `/api/admin/(.*)` 와일드카드는 route handler cache 헤더를 강제 override (사장님 명령 2026-05-10 Fix 28)

- **결함**: Step T 로 Authorization 제거했는데도 prod 응답 `Cache-Control: private, no-store` + `X-Vercel-Cache: BYPASS` 영구. 24초 cold function 호출 매 새로고침 반복.

- **확정 원인 (Vercel 공식 docs)**: https://vercel.com/docs/edge-network/headers
  > "vercel.json 의 `headers` 가 route handler 의 `NextResponse.headers` 와 같은 key 를 set 하면 vercel.json 이 우선."
  와일드카드 `/api/admin/(.*)` 가 `/api/admin/listings` 매칭 → `Cache-Control: private, no-store` 강제 → CDN cache attempt 자체 발생 X (Authorization 제거해도 무관).

- **유효 fix (Fix 28)**:
  와일드카드를 negative lookahead 로 좁힘 — `/api/admin/((?!listings(?:$|/)).*)` 가 `/api/admin/listings` 와 `/api/admin/listings/[id]` 모두 제외.
  그 다음 `/api/admin/listings` 와 `/api/admin/listings/(.*)` 전용 public CDN cache 규칙 추가.
  결과: route handler 의 cache 헤더가 vercel.json 와 충돌 X → CDN HIT 가능.

- **vercel.json header 규칙 절대 순서 (사장님 명령 영구)**:
  1. `/api/admin/((?!listings(?:$|/)).*)` → `private, no-store` (auth/contacts/etc.)
  2. `/api/admin/listings` → public, s-maxage=3600 (LIST endpoint, Fix 28)
  3. `/api/admin/listings/(.*)` → public, s-maxage=300 (single GET, Fix 20)

- **위반 결과**: 새로 admin route 추가 시 와일드카드 `(.*)` 으로 단순화하면 CDN BYPASS 회귀. 반드시 negative lookahead 유지.

- **코드 location**: vercel.json `headers` 배�
### I-PERF-3: 검색은 60K 매물 모두 가능 보장 (사장님 명령 2026-05-11)

- **사장님 정확 인용**: "검색을 하는데 전부다 검색이 되야지 그게 말이 되는 소리야?"
- listing 응답 row 수 줄이는 fix 절대 X (`limit=N` 으로 60K 자르기 X)
- Fix 38 (default limit=5000) 회귀 사례 — 즉시 revert
- 검색 깨지면 즉시 revert
- 첫 진입 빠르게 만들고 싶으면:
  - Progressive (background fetch 으로 60K 다 채움)
  - Server side search (`/api/admin/listings/search?q=`)
- **이 둘 외 path 시 검색 깨짐 발생 → 사장님 비즈니스 critical**

### I-PERF-4: client side fetch wrap 절대 X (사장님 회귀 2026-05-11 Fix 36)

- v294 의 `Object.defineProperty(window, 'fetch', {set: ...})` getter/setter
- 다른 wrapper 가 `window.fetch = newFn` 시도 시 → v294 setter 가 newFn 을 origFetch 로 채택 → v348 wrap 위에 v294 가 outer 됨 → **무한 재귀 위험**
- v341 v1 의 stream tee 충돌도 같은 root cause (clone 위에 clone)
- **fix path**: client wrap X. middleware rewrite 또는 server side 만 사용.
- 위반 결과: 매물 fetch 무한 hang 또는 stack overflow

### I-PERF-5: PostgREST RPC LIMIT 8s timeout 인식 (사장님 회귀 2026-05-11 Fix 36b)

- PostgREST `statement_timeout` = 8s. RPC LIMIT 100000 → JSON serialize 추가 → 8s 초과 → 503
- DB 측정: 5000 rows = 396ms (8s 안 매우 안전)
- 60K rows = 6.9s (8s borderline, prod 에서 timeout 가능)
- **fix path**: chunked LIMIT ≤ 5000 또는 view + LATERAL join 사용
- 단 chunked parallel 도 prod 환경 19s (Fix 36c) — supabase-js + Vercel function combination 비효율

### I-PERF-6: prod 측정 없이 fix 적용 X (사장님 회귀 5번 2026-05-11)

- DB 측정 (psql EXPLAIN ANALYZE) 결과와 prod (supabase-js + Vercel function) 결과 다름
- 1000 rows EXPLAIN 33ms vs prod 응답 18s
- **prod 검증 없이 fix push X** — 사장님 console 명령으로 timing 받은 후 진행
- console 명령 template (`fetch + Date.now()`) — 사장님 시간 1분 안 측정
- 회귀 6번째 발생 시 작업 stop, 다음 세션 신중 진행

### I-SESSION-1: 세션 마스터 기록 (사장님 명령 2026-05-11)

- 큰 변경 작업 후 다음 세션 이어갈 수 있게 마스터 문서 작성
- Location: `docs/sessions/YYYY-MM-DD-<topic>-session-master.md`
- 포함:
  1. 사장님 핵심 명령 (영구)
  2. 현재 prod 상태 (commit SHA + 작동 fix 목록)
  3. revert 된 fix 들 + root cause 분석
  4. 보존된 리소스 (DB function, view, endpoint)
  5. 다음 세션 plan + 옵션
...
### I-CONTACT-1: gongsilclub broker contact 추출 = `ajax_contact_info_gongsil.asp` GET + 0502 안심번호 허용 (사장님 명령 2026-05-14)

- **결함**: 매물 30,000+ 건 모두 `contacts=[]` 빈배열 저장됨. 영업 손실 막대. 사장님이 "월간 1회 한도" 메시지 보고 한도 차감 문제로 추측. 그러나 진짜 root cause 는 다른 곳.

- **확정 원인 (verify v2/v3/v4 prod 측정 2026-05-14)**:
  1. `ajax_contact_info_gongsil.asp` 의 GET fetch path 는 **quota counter 와 무관** (HTML 응답 안에 broker phone 박혀 옴, "초과" alert 텍스트는 button click 시 trigger 되는 JS 함수 정의일 뿐).
  2. 응답에 노출되는 phone 은 **0502 안심번호** (사이트 정책상 real 010 phone 노출 안 함, broker privacy). 0502 로 전화하면 broker 와 정상 통화 가능 → 영업 사용 OK.
  3. `classifyPhone()` 의 `if (!digits.startsWith('010')) return null` filter 가 모든 0502 폐기 → contacts=[] 결과.
  4. v20.1 의 quota_exceeded 사전 감지 regex `/이번달\s*연락처\s*조회\s*횟수를\s*초과/` 는 응답의 `이번 달` (띄어쓰기) 와 매칭 안 됨 — 의도된 broken regex (fix 금지). 매칭되면 contacts=[] 분기 진입.

- **유효 fix (v23 2026-05-14)**:
  1. `classifyPhone()`: `if (!/^(010|0502)/.test(digits)) return null;` — 010 + 0502 허용.
  2. dialog handler 2곳 (`newPage()` + `attachDialogHandler()` popup): 한도 초과 confirm 자동 dismiss (충전 거부 → quota 차감 path 막기).
  3. quota_exceeded regex `이번달` (붙임) 그대로 유지 — fix 금지 (응답 정상 path 도 안에 alert 텍스트 가짐).

- **검증 (verify v4 결과)**: 6 매물 호출 → 5 unique 0502 phone 추출 (`0502-1711-5461 / 0502-5006-6951 / 0502-1411-7594 / 0502-5234-5500 / 0502-4565-7495`). quota counter 미동 (cache 아닌 매물별 real data).

- **위반 결과**: 010 only filter 회귀 시 0502 폐기 → 다시 모든 broker contact 손실 → 영업 전면 중단.

- **코드 location**:
  - `wishes_crawler/gsc_crawl_runner_v20.js` `classifyPhone()`
  - `wishes_crawler/gsc_crawl_runner_v20.js` `newPage()` dialog handler
  - `wishes_crawler/gsc_crawl_runner_v20.js` `attachDialogHandler()` popup dialog handler

- **회귀 검증 (의무)**: `wishes_crawler/VERIFY_V4.bat` 실행 → `차감=0` + phone 추출 6/6 확인. 결과 변경 시 root cause 재진단.

### I-CONTACT-2: onhouse broker phone = `m.onhouse.com/mobile/rent_view/{idx}` HTML parse (사장님 명령 2026-05-14, 17번 verify 잭팟)

- **결함**: onhouse 매물 47,903건 (99.6%) `contacts=[]` 빈배열. 사이트 `phoneView` API quota 강력 적용 (server-side check). 14번 verify (~500+ 호출), 모든 phoneView 변형 path 시도 후에도 quota 우회 불가능 보였음. **사장님 명령 "절대 포기 X" 따라 17번 verify 까지 가서 결국 발견.**

- **확정 우회 path (v15/v16/v17 verify 2026-05-14)**:
  - URL: `https://m.onhouse.com/mobile/rent_view/{idx}` (**mobile sub-domain + `/mobile/rent_view/` path**)
  - **note**: `/mobile/index/rent_view/` 아니라 `/mobile/rent_view/` (URL 패턴 정확)
  - User-Agent: iPhone (`Mozilla/5.0 (iPhone; CPU iPhone OS 17_0 like Mac OS X) AppleWebKit/605.1.15 ...`) 필수
  - 응답 HTML 에 broker 0502 안심번호 + 일부 010 phone **직접 박혀 있음**
  - **quota 차감 0** — `phoneView` API 호출 안 함, page HTML fetch 만으로 phone 받음

- **v17 quota 안전성 100% 확정**:
  - 호출 전 phoneView state: `{"RESULT":"OVER_RANGE"}` (사장님 한도 0)
  - 30 매물 mobile path 대량 호출 + 54 phones 추출 (14.7초)
  - 호출 후 phoneView state: `{"RESULT":"OVER_RANGE"}` (변동 X)
  - = mobile path 와 phoneView 가 **별개 quota counter 사용 X** (mobile path 는 정적 page render, quota check 자체 안 함)

- **v16 매물별 unique 확인**: 7 매물 매물별 unique phone (footer noise 아닌 진짜 broker phone)
  - 3504156 → `0502-1538-0153`
  - 3504141 → `0502-4130-7261`
  - 3254931, 3504173 → `0502-5081-5137` (같은 broker 가 2 매물 등록)
  - 3446671 → 세입자 `010-3455-1920` / 담당자 `010-7121-8725` / 집주인 `010-7577-4000` + 0502-4303-3350
  - 3493241 → 010-7924-4949 + 0502-1606-3376

- **유효 fix (v24 2026-05-14)**:
  1. `lib/onhouse_client.js`: `fetchDetailMobile(session, idx)` 함수 추가 — mobile UA + mobile URL GET
  2. `lib/onhouse_client.js`: `extractPhonesFromMobileHtml(html)` — role + phone regex 매칭 + 0502 정규화
  3. `crawl_runner.js`: phone 추출 path → **mobile primary** (quota 무관), `fetchPhones` fallback (한도 충전 시)
  4. role 매칭 regex: `(사장|사모|관리업체|관리인|임차인|세입자|매도인|매수인|중개사|임대인|담당자|집주인|주인|본인|소유자|부동산)[^\d]{0,40}((?:0502|01[016789]|050\d?)[- .]?\d{3,4}[- .]?\d{4})`
  5. 0502 안심번호 12자리 → `0502-1234-5678` 정규화

- **성능**: 매물당 ~0.49초 (v17 측정). 47,903 매물 전체 first pass 약 6.5시간.

- **위반 결과**: mobile path 회귀 시 47,903 매물 phone 다시 손실 + onhouse 영업 자원 0.4% 회귀. v16/v17 잭팟 path 반드시 보존.

- **코드 location**:
  - `wishes_crawler/lib/onhouse_client.js` `fetchDetailMobile()` + `extractPhonesFromMobileHtml()` + `normalizePhoneFormat()`
  - `wishes_crawler/crawl_runner.js` line 157~ phone 추출 path (mobile primary, phoneView fallback)

- **회귀 검증 (의무)**:
  - `VERIFY_V16.bat` — 7 매물 호출 → 매물별 unique phone 추출 확인
  - `VERIFY_V17.bat` — 30 매물 호출 전/후 phoneView state 동일 (`OVER_RANGE` → `OVER_RANGE`) 확인. quota 차감 0 회귀 시 root cause 재진단.

### I-CONTACT-3: 매물 모달 contacts = listing id strict 검증 필수 (사장님 발견 2026-05-14, CRITICAL)

- **결함**: 매물 A (예: 114206 방배동) 모달 닫고 매물 B (예: 114221 송파구) 모달 열 때, B 모달에 A 의 phone (또는 직전 매물 phone) 잘못 표시. **영업 critical 결함** — 잘못된 broker 와 연결되어 사장님 신뢰 손실.

- **확정 root cause**: `public/search/content-v270-contacts.js` 의 `findCurrentContacts()` 함수가 `window.__currentListing.contacts` 만 보고 **`__currentListing.id` 가 현재 열린 modal 의 listing id 와 일치하는지 검증 안 함**.
  - 매물 A 모달 close 시 `__currentListing` 가 즉시 null 로 reset 안 됨
  - 매물 B 모달 open 시 `__currentListing` 가 A 그대로인 채로 `findCurrentContacts()` 호출 → A 의 contacts 가 B 모달에 노출
  - v270 의 함수는 closure 안이라 직접 override 불가 → wrapper patch 필요

- **유효 fix (v378 2026-05-14)**: `public/search/content-v378-contact-id-strict.js`
  1. **`__currentListing` setter trap** — `Object.defineProperty(window, '__currentListing', {get, set})` 으로 listing 변경 감지 + log
  2. **`getModalListingId()`** — DOM `data-listing-id` / `data-v240-add-contact` 속성으로 현재 modal 의 listing id 확인
  3. **`patchedFindCurrentContacts()`** — `__currentListing.id !== modalId` 시 contacts 반환 X (대신 `{pendingFetchId: modalId}` 반환 → fetch by id 분기)
  4. **DOM MutationObserver** — modal id 변경 감지 시 `.v240-contacts-list`, `.v270-contacts-render` 등 contacts UI 영역 자동 clear ("연락처 불러오는 중..." placeholder)
  5. **observer target**: `body`, attributeFilter: `['data-listing-id', 'data-v240-add-contact']`, childList+subtree

- **page.tsx 등록**: `src/app/search/page.tsx` patches 배열에 `['ws-ext-patch-v378-contact-id-strict', '/search/content-v378-contact-id-strict.js?v=20260514a']` 등록 — v377 이후 last entry

- **위반 결과**: id strict 검증 누락 시 다른 매물 phone 표시 → 사장님이 잘못된 broker 에 전화 → 신뢰 손실 + 영업 기회 손실
- **회귀 검증 (의무)**: prod 매물 A → B 빠르게 전환 후 B 모달의 phone 이 B 의 실제 contacts 인지 확인 (DB 값 cross-check)

- **코드 location**:
  - `public/search/content-v378-contact-id-strict.js` (patch)
  - `public/search/content-v270-contacts.js` (원본 — closure 안이라 직접 수정 X)
  - `src/app/search/page.tsx` patches 배열 마지막
