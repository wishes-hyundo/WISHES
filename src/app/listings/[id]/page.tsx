...
export default async function ListingDetailPage({ params }: Props) {
  ...
  // ì¡°íì ì¦ê° (ìë² í´ë¼ì´ì¸í¸ ì¬ì© - RLS ì°í)
  try {
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="bg-white rounded-xl border border-gray-200 p-6">
              <div className="flex items-center gap-3 mb-1">
                ...
                {listing.views > 0 && (
                  <span className="text-xs text-gray-400 flex items-center gap-1">
                    <Eye className="w-3 h-3" /> ì¡°í {listing.views}
                  </span>
                )}
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4 mt-6">
                ...
                <InfoRow label="ì ì©ë©´ì " value={listing.area_m2 ? `${listing.area_m2}ã¡ (${sqmToPyeong(listing.area_m2)}í)` : 'ì ë³´ ìì'} />
          ...
          <div className="space-y-4">
            <div className="bg-white rounded-xl border border-gray-200 p-4 sm:p-6 lg:sticky lg:top-24">
              ...
              <DetailClientActions listingId={listing.id} />
              ...
              <div className="mt-6 pt-4 border-t border-gray-100 text-xs text-gray-400 space-y-1">
                ...
                <p className="flex items-center gap-1">
                  <Calendar className="w-3 h-3" />
                  ìì ì¼: {new Date(listing.updated_at).toLocaleDateString('ko-KR', { timeZone: 'Asia/Seoul' })}
                </p>
              </div>
            </div>
          </div>
