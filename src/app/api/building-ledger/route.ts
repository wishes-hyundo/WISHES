...
// 소유자정보는 HubService에 없으므로 여러 서비스를 순차 시도
...
async function fetchOwnerInfoWithFallback(
  ...
  throw new Error("소유자정보 조회 실패 (시도: " + errors.join(" | ") + ")");
}
  ...
  try {
    ...
    const results = await Promise.allSettled(
      opsToFetch.map(async (op) => {
        ...
        // 소유자정보는 여러 서비스 URL을 순차 시도 (fallback)
        if (op === "ownerInfo") {
          return await fetchOwnerInfoWithFallback(opName, params);
        }
...
/**
 * 소유자정보 API를 여러 서비스 URL로 순차 시도 (fallback)
 */
async function fetchOwnerInfoWithFallback(
  opName: string,
  ...
  for (const baseUrl of OWNER_API_URLS) {
    try {
      ...
      console.log("[ownerInfo] trying:", baseUrl);
      ...
      try {
        ...
        errors.push(baseUrl + " -> JSON parse failed");
        continue;
      }
      ...
      if (resultCode && resultCode !== "00") {
        errors.push(
          baseUrl + " -> resultCode:" + resultCode + " " + (data?.response?.header?.resultMsg || "")
        );
        continue;
      }
      ...
      console.log("[ownerInfo] success from:", baseUrl, "items:", itemArray.length);
  ...
  // 모든 URL 실패 시
  console.error("[ownerInfo] all URLs failed:", errors);
  throw new Error("소유자정보 조회 실패 (시도: " + errors.join(" | ") + ")");
}

/**
 * 소유자정보에 성별 정보 추가
 * 주민번호 뒷자리 첫번째: 1=남(1900년대), 2=여(1900년대), 3=남(2000년대), 4=여(2000년대)
 */
function processOwnerInfo(items: any[]) {
  return items.map((item: any) => {
    const juminBack = extractGenderDigit(item);
