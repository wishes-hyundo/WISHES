...
// GET /api/listings/map - 지도 bounds 기반 매물 조회
  ...
  try {
    ...
    if (!swLat || !swLng || !neLat || !neLng) {
      return NextResponse.json(
        { success: false, error: 'bounds 파라미터가 필요합니다 (swLat, swLng, neLat, neLng)' },
        { status: 400 }
      );
    }
    ...
    // 필터 조건: bounds + 상태 + 좌표 존재
    const conditions = [
      ne(listings.status, '계약완료'),
      gte(listings.lat, swLat),
      lte(listings.lat, neLat),
      gte(listings.lng, swLng),
      lte(listings.lng, neLng),
    ];
