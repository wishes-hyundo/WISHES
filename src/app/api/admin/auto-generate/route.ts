      ...
      if (addrRes.ok) {
        ...
        if (addr && (addr.sigunguCd || addr.admCd)) {
          ...
          if (!hasValidParcel) {
            ...
            // 건축물대장 API 호출
            // L-fix-internal-bearer (2026-04-28): 사용자 admin 토큰 전달 (req.headers, NOT request)
            const userAuthBld = req.headers.get('authorization') || req.headers.get('Authorization') || '';
            const bldHdrs: Record<string, string> = { 'Content-Type': 'application/json' };
            if (userAuthBld) bldHdrs['Authorization'] = userAuthBld;
