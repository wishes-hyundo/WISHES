...
const dealTypes: DealType[] = ['��┷', '�肥┷', '�､�､'];
const listingTypes: ListingType[] = ['�尖｣ｸ', '妤ｬ�ｸ', '�ｰ�ｬ�ｴ', '�､嵓ｼ�､奛', '��血孖ｸ', '��ｰ', '�ｬ�ｴ�､'];
...
function extractCity(address: string): string {
  ...
  if (parts[0]?.includes('�懍垪')) return '�懍垪';
  if (parts[0]?.includes('�ｸ�')) return '�ｸ�';
  if (parts[0]?.includes('��ｰ')) return '��ｰ';
  if (parts[0]?.includes('��ｬ')) return '��ｬ';
  if (parts[0]?.includes('��')) return '��';
  if (parts[0]?.includes('�卓｣ｼ')) return '�卓｣ｼ';
  if (parts[0]?.includes('�ｸ�ｰ')) return '�ｸ�ｰ';
  if (parts[0]?.includes('�ｸ�')) return '�ｸ�';
  if (parts[0]?.includes('�ｽ�ｰ')) {
    if (parts[1]) return parts[1].replace(/�$/, '') + '�';
    return '�ｽ�ｰ';
  }
  return parts[0]?.replace(/(孖ｹ��亨|�卓溜�忿孖ｹ��梵�們亨|孖ｹ��梵�俯巡|�)$/, '') || '';
}
...
function extractGu(address: string): string {
  ...
  for (const part of parts) {
    if (part.endsWith('�ｬ') || part.endsWith('�ｰ')) return part;
  }
  return parts[1] || '';
}
...
function createPriceMarkerContent(listing: Listing, isSelected: boolean): HTMLElement {
  const priceText = listing.deal === '�､�､' ? formatPrice(listing.price || 0) : listing.deal === '�肥┷' ? `${formatPrice(listing.deposit)}/${listing.monthly}�形 : formatPrice(listing.deposit);
  const colorMap: Record<string, { bg: string; border: string; text: string; dot: string }> = {
    '��┷': { bg: '#EFF6FF', border: '#3B82F6', text: '#1D4ED8', dot: '#3B82F6' },
    '�肥┷': { bg: '#FFF7ED', border: '#F97316', text: '#C2410C', dot: '#F97316' },
    '�､�､': { bg: '#F0FDF4', border: '#22C55E', text: '#15803D', dot: '#22C55E' },
  };
  const colors = colorMap[listing.deal] || colorMap['��┷'];
...
export default function MapSearchPage() {
  ...
  const zoomLevelLabel = useMemo(() => {
    ...
    if (zoomLevel >= 7) return '�ｬ/�ｰ';
    if (zoomLevel >= 5) return '�/�ｭ�ｴ';
    return '�､�ｼ';
  }, [zoomLevel]);

  // �､�ｼ 增ｴ�ｭ 竊 �ｬ�ｼ�ｴ� 甯ｨ� �ｴ�ｰ
  ...
  // �ｴ�ｴ�､�ｵ �一ｸｰ嶹
  useEffect(() => {
    if (!window.kakao?.maps) { console.warn('�ｴ�ｴ�､�ｵ SDK� �罹糖�們ｧ �喜葺�ｵ�壱共.'); setMapReady(true); return; }
    ...
    <div className="pt-20 h-screen flex flex-col">
      ...
      <div className="bg-white border-b border-gray-100 shadow-sm shrink-0">
        <div className="px-4 pt-3 pb-2 flex items-center gap-2">
          <div className="flex gap-1.5 overflow-x-auto no-scrollbar">
            <button onClick={() => setFilters((prev) => ({ ...prev, deal: undefined }))} className={`px-3.5 py-1.5 text-xs font-bold rounded-full border-2 transition-all whitespace-nowrap ${!filters.deal ? 'bg-wishes-primary text-white border-wishes-primary shadow-sm' : 'bg-white text-gray-500 border-gray-200 hover:border-gray-400'}`}>��ｲｴ</button>
            {dealTypes.map((deal) => {
              const dealColors: Record<string, { active: string; ring: string }> = { '��┷': { active: 'bg-blue-500 border-blue-500', ring: 'hover:border-blue-300 hover:text-blue-600' }, '�肥┷': { active: 'bg-orange-500 border-orange-500', ring: 'hover:border-orange-300 hover:text-orange-600' }, '�､�､': { active: 'bg-emerald-500 border-emerald-500', ring: 'hover:border-emerald-300 hover:text-emerald-600' } };
              const c = dealColors[deal] || dealColors['��┷']; const isActive = filters.deal === deal;
          ...
          <div className="ml-auto flex items-center gap-2 shrink-0">
            ...
            <div className="md:hidden flex bg-gray-100 rounded-lg p-0.5">
              <button onClick={() => setMobileView('map')} className={`px-3 py-1 text-xs rounded-md transition-all ${mobileView === 'map' ? 'bg-white shadow text-wishes-primary font-bold' : 'text-gray-500'}`}><MapPin className="w-3 h-3 inline mr-1" />��</button>
              <button onClick={() => setMobileView('list')} className={`px-3 py-1 text-xs rounded-md transition-all ${mobileView === 'list' ? 'bg-white shadow text-wishes-primary font-bold' : 'text-gray-500'}`}><List className="w-3 h-3 inline mr-1" />�ｩ�</button>
            </div>
          </div>
        </div>
        <div className="px-4 pb-2.5 flex gap-1.5 overflow-x-auto no-scrollbar">
          <button onClick={() => setFilters((prev) => ({ ...prev, type: undefined }))} className={`px-3 py-1 text-[11px] font-semibold rounded-full border transition-all whitespace-nowrap ${!filters.type ? 'bg-gray-800 text-white border-gray-800' : 'bg-gray-50 text-gray-500 border-gray-200 hover:border-gray-400'}`}>��ｲｴ</button>
      ...
      <div className="flex-1 flex overflow-hidden">
        <div className={`relative ${mobileView === 'list' ? 'hidden md:block' : ''} flex-1`}>
          ...
          {loading && (<div className="absolute top-4 left-1/2 -translate-x-1/2 bg-white/90 backdrop-blur-md px-4 py-2 rounded-full shadow-lg flex items-center gap-2 text-sm z-20"><Loader2 className="w-4 h-4 animate-spin text-wishes-secondary" />�､�ｼ �� �...</div>)}
          {!loading && mapReady && (
            <div className="absolute top-4 left-4 flex flex-col gap-2 z-20">
              <div className="bg-white/95 backdrop-blur-md px-3 py-1.5 rounded-full shadow-md text-xs font-medium text-gray-700 flex items-center gap-2"><span>嶸�椪 �� ��溜</span><strong className="text-wishes-primary">{total}</strong>�ｴ</div>
              <div className="bg-white/90 backdrop-blur-md px-3 py-1 rounded-full shadow text-[10px] font-semibold text-wishes-muted flex items-center gap-1.5"><div className={`w-2 h-2 rounded-full ${zoomLevel >= 9 ? 'bg-green-700' : zoomLevel >= 7 ? 'bg-blue-500' : zoomLevel >= 5 ? 'bg-green-500' : 'bg-gray-500'}`} />{zoomLevelLabel} �ｨ� 岺懍亨</div>
            </div>
          )}
        </div>
          ...
          <div ref={sidebarRef} style={{ transform: detailId ? 'translateX(-100%)' : 'translateX(0)' }} className="absolute inset-0 overflow-y-auto custom-scrollbar transition-transform duration-300 ease-in-out">
            <div className="p-4 space-y-3">
              <div className="flex items-center justify-between">
                <div className="text-sm font-medium text-gray-700">�､�ｼ <strong className="text-wishes-primary">{searchQuery ? filteredListings.length : total}</strong>�ｴ{searchQuery && (<span className="ml-1 text-xs text-wishes-muted">&quot;{searchQuery}&quot; ��賀ｲｰ�ｼ</span>)}</div>
              </div>
              {filteredListings.length > 0 ? (
                ...
                <div className="text-center py-16 text-gray-400">
                  ...
                  <p className="text-sm font-medium">{loading ? '�� �...' : searchQuery ? '�� �ｰ�ｼ� ��慣�壱共' : '�ｴ ��溜� �､�ｼ�ｴ ��慣�壱共'}</p>
                  {searchQuery && (<button onClick={() => setSearchQuery('')} className="mt-2 text-xs text-wishes-secondary hover:underline">��餓牟 �一ｸｰ嶹</button>)}
                </div>
              )}
            </div>
          </div>
