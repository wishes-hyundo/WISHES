...
function createOfficeMarkerContent(): HTMLElement {
  ...
  icon.style.fontFamily = "'GmarketSans', sans-serif";
  ...
  wrapper.addEventListener('mouseenter', () => {
    wrapper.style.transform = 'translate(-50%, -100%) scale(1.15)';
  });
...
function createPriceMarkerContent(listing: Listing, isSelected: boolean): HTMLElement {
  ...
  content.style.cssText = `
    ...
    white-space: nowrap;
    cursor: pointer;
    box-shadow: ${isSelected ? `0 4px 12px rgba(0,0,0,0.25)` : `0 2px 6px rgba(0,0,0,0.15)`};
  ...
  tail.style.cssText = `
    ...
    border-top: 7px solid ${isSelected ? colors.border : colors.border};
  `;
...
export default function MapSearchPage() {
  ...
  useEffect(() => {
    ...
    window.kakao.maps.load(() => {
      ...
      const officeOverlay = new window.kakao.maps.CustomOverlay({
  ...
  // ─────── 마카 업데이트 — 줌 레벨에 따라 단계별 전환 ───────
  useEffect(() => {
    if (!mapInstanceRef.current) return;
    const map = mapInstanceRef.current;

    // 기존 오버레이 전부 제거 (사무소 마카 제외)
    ...
    if (level >= 9) {
      ...
      // ─────── 개별 매물 마카 (줌의 상태) — 겹침 방지 로직 ───────
      const positionGroups: Record<string, Listing[]> = {};
      validListings.forEach((listing) => {
        // 소수점 5자리 기준 그룹핑 (~1m 반경)
        const key = `${listing.lat!.toFixed(5)}_${listing.lng!.toFixed(5)}`;
        if (!positionGroups[key]) positionGroups[key] = [];
        positionGroups[key].push(listing);
      });
      ...
      Object.values(positionGroups).forEach((group) => {
        group.forEach((listing, idx) => {
          ...
          // 같은 위치에 여러 매물이 있으면 원형으로 분산 배치
          if (group.length > 1) {
            const angle = (2 * Math.PI * idx) / group.length;
            const offset = 0.00012; // 약 12m 반경으로 분산
          ...
          content.addEventListener('click', () => {
            ...
            map.panTo(new window.kakao.maps.LatLng(listing.lat!, listing.lng!));
          });
  ...
  // ─────── 매물 마카 클릭 핸들러 (사이드바 연동) ───────
  const handleMarkerClick = useCallback(
    (id: number) => {
      setSelectedId(id);
      // 모바일에서는 자동으로 리스트 뷰로 전환
    ...
    <div className="pt-20 h-screen flex flex-col">
            ...
            {dealTypes.map((deal) => {
              const dealColors: Record<string, { active: string; ring: string }> = {
                ...
                '월세': {
                  ...
                  ring: 'hover:border-orange-300 hover:text-orange-600',
                },
                '매매': {
                  active: 'bg-emerald-500 border-emerald-500',
                  ring: 'hover:border-emerald-300 hover:text-emerald-600',
                },
              };
            ...
            <div className="md:hidden flex bg-gray-100 rounded-lg p-0.5">
              <button
                ...
                className={`px-3 py-1 text-xs rounded-md transition-all ${
                  mobileView === 'map'
                    ? 'bg-white shadow text-wishes-primary font-bold'
                    : 'text-gray-500'
                }`}
              >
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div
          ...
          } w-full md:w-[380px] bg-white border-l border-gray-200 overflow-y-auto custom-scrollbar shrink-0`}
        >
                ...
                <div
                  ...
                  className={`rounded-xl transition-all duration-200 ${
                    selectedId === listing.id
                      ? 'ring-2 ring-wishes-secondary shadow-lg scale-[1.02]'
                      : ''
                  }`}
                >
                  <ListingCard
