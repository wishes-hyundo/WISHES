...
const STEPS = [
  { id: 1, label: '필수정보', icon: 'ð', desc: '소재지·거래·유형' },
  { id: 2, label: '건축물대장', icon: 'ð️', desc: '자동조회·세부정보' },
  { id: 3, label: '사진등록', icon: 'ð¸', desc: '이미지·품질개선' },
  { id: 4, label: 'AI등록', icon: 'ð¤', desc: '자동완성·업로드' },
];
...
function generateStyledDescription(form: FormData, buildingInfo: BuildingInfo | null, style: AiStyle): string {
  ...
  switch (style) {
    case 'trendy': {
      ...
      if (dong) lines.push(`ð 위치: ${dong}${form.addressDetail ? ' ' + form.addressDetail : ''}`);

      if (form.deal === '월세') {
        lines.push(`ð° 월세 ${form.deposit ? form.deposit + '/': ''}${form.monthly || ''}만원`);
      } else if (form.deal === '전세') {
        lines.push(`ð° 전세 ${form.deposit || ''}만원`);
      } else if (form.deal === '매매') {
        lines.push(`ð° 매매 ${form.price || ''}만원`);
      }

      if (form.area_m2) lines.push(`ð 전용면적 ${form.area_m2}㎡${form.area_supply_m2 ? ` (공급 ${form.area_supply_m2}㎡)` : ''}`);
      if (form.type) lines.push(`ð  매물유형: ${form.type}`);
      lines.push('');

      // 조건부 항목
      if (hasStation) lines.push(`ð ${station ? station[1] : '지하철역'} 도보 이용 가능`);
      if (form.features.includes('풀옵션')) lines.push(pick(['ð  풀옵션 (에어컨·냉장고·세탁기·인덕션)', 'ð  옵션 완비 - 입주 시 추가 비용 없음!']));
      if (isNew) lines.push(pick(['✨ 깔끔한 신축 컨디션', '✨ 신축 건물로 상태 최상!']));
      if (form.parking_available || form.features.includes('주차가능')) lines.push('ð¿️ 주차 가능');
      if (form.features.includes('엘리베이터') || (buildingInfo && buildingInfo.승용엘리베이터 > 0)) lines.push('ð 엘리베이터 완비');
      if (form.direction === '남향' || form.direction === '남동향') lines.push(pick(['☀️ 남향 채광 최고', '☀️ 햇살 가득한 남향 배치']));
      if (form.features.includes('CCTV')) lines.push('ð¹ CCTV 보안');
      if (form.features.includes('무인택배')) lines.push('ð¦ 무인택배함');
      if (form.features.includes('반려동물') || form.pet_allowed) lines.push('ð¾ 반려동물 OK');
      if (form.floor_current) lines.push(`ð¢ ${form.floor_current}층${form.floor_total ? '/' + form.floor_total + '층' : ''}`);
      if (form.rooms) lines.push(`ð️ 방 ${form.rooms}개${form.bathrooms ? ' / 화장실 ' + form.bathrooms + '개' : ''}`);
      ...
      const closings = [
        'ð 문의주세요! 상담 바로 가능합니다.',
        'ð 빠른 입주 원하시면 지금 문의하세요!',
        'ð 상담 문의 환영합니다. 빠른 답변 드리겠습니다!',
        'ð 좋은 조건, 만족스러운 입주! 문의하세요.',
        'ð 상세 사진과 정보는 문의 시 안내드립니다!',
      ];
      ...
      <div className="bg-white border-b sticky top-0 z-40">
        <div className="max-w-6xl mx-auto px-6 py-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            {lastSavedAt && (
              <span className="text-xs text-gray-400 hidden sm:inline">ð¢ {lastSavedAt} 저장됨</span>
            )}
          </div>
        </div>
      ...
      {showDrafts && (
        <div className="fixed inset-0 z-30" onClick={() => setShowDrafts(false)}>
          <div className="absolute top-24 right-6 w-96 bg-white rounded-xl shadow-2xl border max-h-[500px] overflow-y-auto"
            ...
            <div className="p-4 border-b font-semibold text-gray-800">ð 임시저장 목록 ({drafts.length}건)</div>
            {drafts.length === 0 ? (
      ...
      <div role="main" aria-label="매물 등록 폼" className="max-w-6xl mx-auto px-6 py-8">
            ...
            <div className="bg-white rounded-2xl shadow-sm border p-8">
              ...
              <div className="mb-8">
                <label className="block text-sm font-semibold text-gray-700 mb-2">
                  ð 소재지 <span className="text-red-500">*</span>
                </label>
              ...
              <div className="mb-8">
                <label className="block text-sm font-semibold text-gray-700 mb-2">
                  ð° 거래유형 및 가격 <span className="text-red-500">*</span>
                </label>
                ...
                {(form.deposit || form.monthly || form.price) && (
                  <div className="mt-2 text-sm text-green-700 font-medium">
                    ðµ {form.deal === '매매' ? `매매가 ${formatAmount(form.price)}` :
              ...
              <div className="mb-6">
                <label className="block text-sm font-semibold text-gray-700 mb-2">
                  ð  매물유형 <span className="text-red-500">*</span>
                </label>
        ...
        {currentStep === 2 && (
          <div className="step-content animate-[fadeIn_0.3s_ease-in-out] grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                {buildingInfo && !showBuildingDoc && (
                  <div className="space-y-4 text-sm">
                    ...
                    <div className="bg-gray-50 rounded-xl p-4">
                      <h3 className="font-semibold text-gray-800 mb-2">ð¢ 건물 기본정보</h3>
                    ...
                    {/* 면적/비율 */}
                    <div className="bg-gray-50 rounded-xl p-4">
                      <h3 className="font-semibold text-gray-800 mb-2">ð 면적 · 비율</h3>
                      <div className="grid grid-cols-2 gap-2 text-gray-600">
                    ...
                    {/* 층수/승강기/주차 */}
                    <div className="bg-gray-50 rounded-xl p-4">
                      <h3 className="font-semibold text-gray-800 mb-2">ð¢ 층수 · 시설</h3>
                      <div className="grid grid-cols-2 gap-2 text-gray-600">
                    ...
                    {buildingInfo.층별개요 && buildingInfo.층별개요.length > 0 && (
                      <div className="bg-gray-50 rounded-xl p-4">
                        <h3 className="font-semibold text-gray-800 mb-2">ð 층별개요</h3>
            ...
            <div className="space-y-6">
              <div className="bg-white rounded-2xl shadow-sm border p-6">
                <h3 className="font-bold text-gray-900 mb-4">ð 세부정보 (수정 가능)</h3>
            ...
            {showAiPanel && (
              <div className="bg-white rounded-2xl shadow-sm border p-8 space-y-6">
                <div className="flex items-center justify-between">
                  <h3 className="text-lg font-bold text-gray-900 flex items-center gap-2">ð¤ AI 자동등록</h3>
                ...
                {!form.title && (
                  <div className="text-center">
                    <button onClick={() => runAiAutoFill(aiStyleOption, aiModel)} disabled={aiGenerating} className="px-10 py-3.5 bg-green-700 text-white rounded-xl font-semibold hover:bg-green-800 transition shadow-lg disabled:bg-gray-400 text-base">
                      {aiGenerating ? (<span className="flex items-center gap-2"><div className="animate-spin w-5 h-5 border-2 border-white border-t-transparent rounded-full" />AI가 매물 정보를 분석 중...</span>) : 'ð¤ AI 자동완성 실행'}
                    </button>
                  </div>
                )}
                {form.title && (
                  <>
                    <div>
                      <div className="flex items-center justify-between mb-2">
                        <label className="text-sm font-semibold text-gray-700">ð 매물 제목 <span className="text-green-600 text-xs font-normal ml-1">AI 생성됨</span></label>
                        <button onClick={() => runAiAutoFill(aiStyleOption, aiModel)}
                          ...
                          {aiGenerating ? '생성 중...' : 'ð 다시 생성'}</button>
                      </div>
                    ...
                    <div>
                      <label className="block text-sm font-semibold text-gray-700 mb-2">ð 매물 설명 <span className="text-green-600 text-xs font-normal ml-1">AI 생성됨</span></label>
                    ...
                    <div className="bg-gray-50 rounded-xl p-5">
                      <h4 className="font-semibold text-gray-800 text-sm mb-3">ð 등록 정보 요약</h4>
          ...
          <div className="space-y-6 step-content animate-[fadeIn_0.3s_ease-in-out]">
            <div className="bg-white rounded-2xl shadow-sm border p-8">
              ...
              <div className="bg-gray-50 rounded-xl p-5 mb-6">
                <h3 className="font-semibold text-gray-800 text-sm mb-3">ð 등록 정보 요약</h3>
                <div className="grid grid-cols-2 md:grid-cols-3 gap-3 text-sm">
