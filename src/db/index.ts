...
function autoSeed() {
  ...
  for (const listing of sampleListings) {
    ...
    // 이미지 삽입
    const urls = imageUrls[listing.imageType as keyof typeof imageUrls] || imageUrls.원룸;
...
// 초기 시드 체크 및 실행
try {
  const result = sqlite.prepare('SELECT COUNT(*) as count FROM listings').get() as { count: number };
