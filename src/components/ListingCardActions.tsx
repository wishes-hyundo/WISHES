...
export default function ListingCardActions({ listingId }: ListingCardActionsProps) {
  ...
  const handleFavorite = (e: React.MouseEvent) => {
    ...
    const msg = isFavorite(listingId) ? '찜 목록에서 제거되었습니다' : '찜 목록에 추가되었습니다';
    setShowToast(msg);
  ...
  const handleCompare = (e: React.MouseEvent) => {
    ...
    if (result === false && !isInCompare(listingId)) {
      setShowToast('비교는 최대 4개까지 가능합니다');
    } else {
      const msg = isInCompare(listingId) ? '비교 목록에서 제거' : '비교 목록에 추가';
      setShowToast(msg);
    }
    setTimeout(() => setShowToast(null), 2000);
  };
    ...
    <>
      <div className="absolute top-2 right-2 flex gap-1 z-10">
        <button
          ...
          title="찜"
        >
          <svg xmlns="http://www.w3.org/2000/svg" className="w-4 h-4" fill={isFavorite(listingId) ? 'currentColor' : 'none'} viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
