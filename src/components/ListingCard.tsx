    ...
    return (
      <Link href={`/listings/${listing.id}`} className="group flex bg-white rounded-xl border border-gray-100 overflow-hidden hover:shadow-md hover:border-wishes-secondary/30 transition-all h-28" onMouseEnter={() => onHover?.(listing.id)} onMouseLeave={() => onHover?.(null)}>
        <div className="w-28 h-28 shrink-0 relative overflow-hidden bg-gray-100">
          {thumbUrl ? <img src={thumbUrl} alt={listing.title} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-300" /> : <div className="w-full h-full flex items-center justify-center bg-gray-100"><Building2 className="w-8 h-8 text-gray-300" /></div>}
        ...
        <div className="flex-1 p-3 flex flex-col justify-between min-w-0">
          ...
          <div className="flex items-center gap-2 text-xs text-wishes-muted">
            ...
            {(listing as any).views > 0 && (<><span>·</span><span className="flex items-center gap-0.5"><Eye className="w-3 h-3" />{(listing as any).views}</span></>) }
          </div>
        </div>
      </Link>
    );
  ...
  return (
    <Link href={`/listings/${listing.id}`} className="group card-premium block overflow-hidden" onMouseEnter={() => onHover?.(listing.id)} onMouseLeave={() => onHover?.(null)}>
      <div className="relative overflow-hidden bg-gradient-to-br from-gray-200 to-gray-300 aspect-[16/10]">
        {thumbUrl ? <img src={thumbUrl} alt={listing.title} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500 ease-out" loading="lazy" /> : <div className="w-full h-full flex items-center justify-center bg-gradient-to-br from-gray-100 to-gray-200"><Building2 className="w-12 h-12 text-gray-400" /></div>}
        ...
        <div className="absolute top-3 left-3 flex flex-col gap-1.5">
          <span className={cn('px-3 py-1 text-xs font-bold rounded-lg shadow-lg backdrop-blur-sm w-fit', getDealColor(listing.deal))}>{listing.deal}</span>
          {isNew(listing.created_at) && <span className="flex items-center gap-0.5 px-2 py-1 text-xs font-bold bg-yellow-400 text-yellow-900 rounded-lg shadow-sm w-fit"><Sparkles className="w-3 h-3" /> NEW</span>}
          {isHot((listing as any).views) && <span className="flex items-center gap-0.5 px-2 py-1 text-xs font-bold bg-red-500 text-white rounded-lg shadow-sm w-fit"><Flame className="w-3 h-3" /> HOT</span>}
        </div>
        <button onClick={(e) => { e.preventDefault(); e.stopPropagation(); toggleFavorite(listing.id); }} className="absolute top-3 right-3 w-9 h-9 rounded-full bg-white/80 backdrop-blur-sm flex items-center justify-center shadow-md hover:bg-white transition-all z-10" aria-label={liked ? '찜 해제' : '찜하기'}>
          <Heart className={cn('w-4.5 h-4.5 transition-colors', liked ? 'fill-red-500 text-red-500' : 'text-gray-400')} />
        </button>
        <div className="absolute bottom-3 right-3">
          <span className="px-3 py-1 text-xs font-semibold bg-white/90 text-wishes-primary rounded-lg shadow-md backdrop-blur-sm">{listing.type}</span>
        </div>
      </div>
      <div className="p-4 space-y-4">
        ...
        <div className="flex items-center gap-4 text-xs text-wishes-muted">
          {(listing.area_m2 || listing.area) ? <div className="flex items-center gap-1"><Maximize className="w-4 h-4 text-wishes-secondary/60" /><span>{listing.area_m2 || listing.area}㎡</span><span className="text-gray-400">({sqmToPyeong(listing.area_m2 || listing.area)}평)</span></div> : null}
          {(listing.floor_current || listing.floor) && <div className="flex items-center gap-1"><Building2 className="w-4 h-4 text-wishes-secondary/60" /><span>{formatFloor(listing)}</span></div>}
        </div>
        ...
        <div className="flex flex-wrap gap-2 pt-2">
          {listing.parking && <span className="px-2.5 py-1 text-xs font-medium bg-wishes-secondary/10 text-wishes-secondary rounded-full border border-wishes-secondary/20 hover:bg-wishes-secondary/20 transition-colors">🚗 주차</span>}
          {listing.elevator && <span className="px-2.5 py-1 text-xs font-medium bg-wishes-accent/10 text-wishes-accent rounded-full border border-wishes-accent/20 hover:bg-wishes-accent/20 transition-colors">💡 엘리베이터</span>}
          {listing.pet && <span className="px-2.5 py-1 text-xs font-medium bg-emerald-500/10 text-emerald-600 rounded-full border border-emerald-500/20 hover:bg-emerald-500/20 transition-colors">🐾 반려동물</span>}
        </div>
        <div className="pt-3 border-t border-gray-100 flex items-center justify-between text-xs">
          <div className="flex items-center gap-3">
            <span className="text-wishes-muted font-mono flex items-center gap-1"><Hash className="w-3 h-3" />W-{listing.id}</span>
            {(listing as any).views > 0 && <span className="text-wishes-muted flex items-center gap-1"><Eye className="w-3 h-3" />{(listing as any).views}</span>}
          </div>
          <span className="text-wishes-muted flex items-center gap-1"><Calendar className="w-3 h-3" />{listing.created_at ? new Date(listing.created_at).toLocaleDateString('ko-KR', { timeZone: 'Asia/Seoul', month: 'short', day: 'numeric' }) : '방금 전'}</span>
        </div>
      </div>
    </Link>
  );
