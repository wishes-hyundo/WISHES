      ...
      >
        ...
        <div className="flex-1 p-3 flex flex-col justify-between min-w-0">
          ...
          <div className="flex items-center gap-2 text-xs text-wishes-muted flex-wrap">
            ...
            {/* ììì©: ìì¢ íê·¸ */}
            {(listing.type === 'ìê°' || listing.type === 'ì¬ë¬´ì¤') && (listing as any).business_type && (
            ...
            {/* ììì©: íë¹ê° */}
            {(listing.type === 'ìê°' || listing.type === 'ì¬ë¬´ì¤') && listing.area_m2 && listing.monthly && (
              <>
                <span>Â·</span>
                <span className="text-purple-600 font-semibold">íë¹ {Math.round(listing.monthly / (listing.area_m2 / 3.3058)).toLocaleString()}ë§</span>
              </>
            )}
  ...
  return (
    ...
    >
      ...
      <div className="p-4 space-y-4">
        ...
        {/* ììì© ì¶ê° ì ë³´ */}
        {(listing.type === 'ìê°' || listing.type === 'ì¬ë¬´ì¤') && (
          <div className="flex flex-wrap gap-1.5">
            ...
            {listing.area_m2 && listing.monthly && (
              <span className="text-[10px] px-2 py-0.5 bg-blue-50 text-blue-600 rounded-full font-semibold">
                íë¹ {Math.round(listing.monthly / (listing.area_m2 / 3.3058)).toLocaleString()}ë§
              </span>
            )}
            {(listing as any).goodwill_fee > 0 && (
              <span className="text-[10px] px-2 py-0.5 bg-amber-50 text-amber-700 rounded-full font-semibold">
                ê¶ë¦¬ê¸ {formatAmount((listing as any).goodwill_fee)}
              </span>
            )}
            {(listing as any).vat_included === false && listing.deal === 'ìì¸' && (
              <span className="text-[10px] px-2 py-0.5 bg-red-50 text-red-600 rounded-full font-semibold">ë¶ê°ì¸ë³ë</span>
            )}
          </div>
        )}
        ...
        <div className="flex items-center gap-1 text-xs text-wishes-muted">
          ...
          <span className="truncate">{listing.dong || ''}{listing.dong && listing.address ? ' Â· ' : ''}{listing.address ? listing.address.split(' ').slice(-1)[0] : ''}</span>
        </div>
